...
<div class="flex flex-row min-h-screen justify-center items-center">
	<div class="flex flex-col justify-center card bg-inital card-hover overflow-hidden py-10 px-12 w-[375px] md:w-[600px] lg:w-[600px]">
        ...
        <form method="POST" class="flex flex-col" id="verifyForm">
            <label class="label">
                <span>Email</span>
                <input form="verifyForm" name="email" class={((form?.errors?.email || form?.errors?.not_registered) ? "input input-error" : "input")} type="text" placeholder="email" value={form?.data?.email ?? ""} />
                {#if form?.errors?.email}
                <span class="badge variant-filled-error">{form?.errors?.email[0]}</span>
                {/if}
                {#if form?.errors?.not_registered}
                <span class="badge variant-filled-error">{form?.errors?.not_registered}</span>
                {/if}
            </label>
